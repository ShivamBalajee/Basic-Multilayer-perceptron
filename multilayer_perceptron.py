...
class MultilayerPerceptron:
    ...
    def __init__(self, n_hidden=16, hidden_activation='sigmoid', n_iterations=1000, learning_rate=0.01):
        ...
        self._activation_func = hidden_activation
    ...
    def _initialize(self, X, y):
        ...
        # Implemented Xavier weight and bias initialisation
        ...
        self._h_weights = np.random.uniform(-np.sqrt(factor / (self._X.shape[1] + self.n_hidden)),
                                            np.sqrt(factor / (self._X.shape[1] + self.n_hidden)),
                                            (self._X.shape[1], self.n_hidden))
        self._h_bias = np.random.uniform(-np.sqrt(factor / (self._X.shape[1] + self.n_hidden)),
                                         np.sqrt(factor / (self._X.shape[1] + self.n_hidden)), (1, self.n_hidden))

        self._o_weights = np.random.uniform(-np.sqrt(factor / (self.n_hidden+ self._y.shape[1])),
                                            np.sqrt(factor / (self.n_hidden+ self._y.shape[1])),(self.n_hidden, self._y.shape[1]))
        self._o_bias = np.random.uniform(-np.sqrt(factor / (self.n_hidden+ self._y.shape[1])),
                                         np.sqrt(factor / (self.n_hidden+ self._y.shape[1])),(1, self._y.shape[1]))
    ...
    def fit(self, X, y):
        ...
        _y = one_hot_encoding(y)
        for i in range(self.n_iterations):
            layer_inputs, layer_outputs = self._layer_predict(X)
            error_output = (layer_outputs[-1] - _y)
            # output_gradient = np.dot(error_output, hidden_output)

            self._o_weights -= self.learning_rate * layer_outputs[-2].T @ error_output / X.shape[0]
            self._o_bias -= self.learning_rate * np.mean(error_output, axis=0)

            Z = (error_output @ self._o_weights.T) * self.hidden_activation(layer_inputs[-2], derivative=True)
            self._h_bias -= self.learning_rate * np.mean(Z, axis=0)
            self._h_weights -= self.learning_rate * (X.T @ Z) / X.shape[0]

            if i % 20 == 0:
                iter.append(i)
                loss.append(cross_entropy(_y, layer_outputs[-1]))
    ...
    def _layer_predict(self, X):
        a = []
        z = []
        a.append(X @ self._h_weights + self._h_bias)
        z.append(self.hidden_activation(a[-1]))
        a.append(z[-1] @ self._o_weights + self._o_bias)
        z.append(self._output_activation(a[-1]))
        return a, z
    ...
    def predict(self, X):
        ...
        """
        return np.argmax(self._layer_predict(X)[1][-1], axis=1)
